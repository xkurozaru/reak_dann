...
def main():
    ...
    model = DCNN(len(dataset.classes))
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for (inputs_src, labels, domains), (inputs_tgt, _) in tqdm(
            zip(dataloader, dataloader_tgt), total=len(dataloader_tgt)
        ):
            ...
            outputs_c, outputs_d = model(inputs_src)
            loss_c = criterion_class(outputs_c, labels)
            loss_d = criterion_domain(outputs_d, domains)

            _, output_tgt = model(inputs_tgt)
            ...
            domains = torch.cat((domains, domains_tgt), dim=0)
